...
for line in open(pjoin(local_path,'COMPATIBILITY')):
    mgamc_match   = re.match(r'^MIN_MGAMC_VERSION\s*(?P<version>[\d\.]+)\s*$',line)
    pythia8_match = re.match(r'^MIN_PYTHIA8_VERSION\s*(?P<version>[\d\.]+)\s*$',line)
    if mgamc_match:
...
mg5amc_version_finder = re.compile(r'^\s*version\s*\=\s*(?P<version>[\d\.]+)\s*$')
mg5amc_version = None
